...
public class WhaleFoam : MonoBehaviour
{
    ...
    IEnumerator FoamSpawn()
    {
        ...
        if (isLeft)
        {
            spawn = Instantiate(foams[0], transform.position, Quaternion.Euler(90f, -90f, 0f));
        }
        else
        {
            spawn = Instantiate(foams[0], transform.position, Quaternion.Euler(-90f, 90f, 0));
        }
